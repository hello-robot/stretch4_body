...
def get_hello_ttyACMx_mapping():
    mapping={}
    mapping['hello'] = {'hello-motor-arm': None, 'hello-motor-omni-0': None, 'hello-motor-omni-1': None,'hello-power-periph': None, 'hello-motor-omni-2': None, 'hello-motor-lift': None, 'hello-esp32': None,'hello-pixart-j3':None}
    mapping['missing']=[]
    mapping['ACMx']={}
...
def find_steppers_on_bus():
    s = []
    device_names = ['/dev/hello-motor-arm', '/dev/hello-motor-lift', '/dev/hello-motor-omni-0',
                    '/dev/hello-motor-omni-1','/dev/hello-motor-omni-2']
...
def find_arduinos():
    ...
    for dev in all:
        if dev.idVendor == 0x239a and dev.idProduct == 0x8022:
            devs.append(dev)
    return devs
...
def add_arduino_udev_line_etc(device_name, serial_no):
    ...
    x_out = ''
    overwrite = False
    uline = 'KERNEL=="ttyACM*", ATTRS{idVendor}=="239a", ATTRS{idProduct}=="8022",MODE:="0666", ATTRS{serial}=="' + serial_no + '", SYMLINK+="' + device_name + '", ENV{ID_MM_DEVICE_IGNORE}="1"\n'
    for xx in x:
...
def add_ftdi_udev_line(device_name, serial_no, fleet_dir):
    f = open(fleet_dir + 'udev/99-hello-feetech.rules', 'r')
    x = f.readlines()
    x_out = ''
    overwrite = False
    ...
    if not overwrite:
        ...
        x_out = x_out + uline + '\n'
    f.close()
    f = open(fleet_dir + 'udev/99-hello-feetech.rules', 'w')
    f.write(x_out)
    f.close()
