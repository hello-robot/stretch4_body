...
class SyncTransactionHandler():
    ...
    def do_rpc(self, push, payload, reply_callback, exiting=False):
        ...
        try:
            ...
            self.logger.error("IOError({0}): {1}".format(e.errno, e.strerror))
...
class TransportPySerial():
    ...
    def _unlock_serial(self):
        try:
            fcntl.flock(self.ser.fileno(), fcntl.LOCK_UN | fcntl.LOCK_NB)
        except IOError as e:
            self.logger.error('Port %s is unable to unlock: %s' % (self.port_name, e))
        except Exception as e: 
            self.logger.error('Failed to unlock serial port %s: %s' % (self.port_name, e))
    def _lock_serial(self):
        try:
            fcntl.flock(self.ser.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
            return True
        except IOError as e:
            self.logger.error('Port %s is busy: %s' % (self.port_name, e))
            self.logger.error('Check if Stretch Body Server is running: stretch_body_server --status')
        except Exception as e: 
            self.logger.error('Failed to lock serial port %s: %s' % (self.port_name, e))
    ...
    def configure_version(self, firmware_version):
        ...
        device = xl[0]
        if not (device == 'Stepper2' or device == 'Stepper' or device == 'Wacc' or device == 'Pimu' or device == 'PowerPeriph' or device == 'Pimu2' or device == 'hello-stepper2' or device == 'hello-pimu2' or device == 'hello_stepper2' or device == 'hello_pimu2'):
