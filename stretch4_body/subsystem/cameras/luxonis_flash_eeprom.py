...
def read_calibration_and_write_to_eeprom(camera_type: RGBCameras, clear_eeprom: bool):
    logger.info(f"Connecting to device to retrieve calibration for {camera_type}...")
    ...
    with dai.Device(dai.UsbSpeed.SUPER_PLUS) as device:
        ...
        if clear_eeprom:
            logger.info("Clearing EEPROM before writing...")
            device.factoryResetCalibration()
            logger.info("EEPROM cleared.")
    ...
    Writing the following calibration: {new_config}
        ...
        if answer[0] != "y":
            logger.info("Aborting without writing to EEPROM.")
            return
        
        eeepromData = calibration_handler.getEepromData()
        logger.info(f'EEPROM VERSION being flashed is {eeepromData.version}')
        ...
        logger.info(f"EEPROM backup saved to {file_name}")
        
        logger.info("Writing calibration to EEPROM...")
        ...
        logger.info("Done writing to EEPROM.")
