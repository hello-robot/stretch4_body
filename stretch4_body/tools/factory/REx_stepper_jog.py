...
def step_interaction():
    ...
    if len(x)>1:
        ...
        if x[0]=='x':
            sp=float(x[1:])
            motor.set_command(x_des=sp,stiffness=stiffness)
        if x[0]=='v':
            sp=float(x[1:])
            motor.set_command(v_des=sp,stiffness=stiffness)
        if x[0]=='a':
            sp=float(x[1:])
            motor.set_command(a_des=sp)
        if x[0]=='s':
            stiffness=float(x[1:])
            print('New stiffness: ',stiffness)
            motor.set_command(stiffness=stiffness)
        if x[0]=='i':
            ii=float(x[1:])
