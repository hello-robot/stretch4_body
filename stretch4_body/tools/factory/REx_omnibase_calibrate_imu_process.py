...
def write_configuration_params(cma_result):
    ...
    imu = stretch4_body.subsystem.power_periph.IMU()
    params = cma_result['best_parameters']

    A=np.array(params[3:6])
    B = np.array([0.0, 0.0, 1.0])
    R=align_vectors(A,B)
    imu.write_configuration_param_to_YAML('imu.config.mag_offsets',[params[0],params[1],params[2]])
    imu.write_configuration_param_to_YAML('imu.config.mag_softiron_matrix',[
        float(R[0][0]),
        float(R[0][1]),
        float(R[0][2]),
        float(R[1][0]),
        ...
        float(R[2][1]),
        float(R[2][2])])
    imu.write_configuration_param_to_YAML('imu.config.gyro_zero_offsets', [0,0,0])
    imu.write_configuration_param_to_YAML('imu.config.rate_gyro_vector_scale', cma_result['rate_gyro_scale'])
    imu.write_configuration_param_to_YAML('imu.config.gravity_vector_scale', cma_result['gravity_scale'])
...
class MobileBaseImuCalibrator:
    ...
    def load_data(self):
        if self.validation_run:
            filenames = glob.glob(self.calibration_directory + hu.get_fleet_id()+'_omnibase_imu_calibration_data_validate_' + '[0-9]*[0-9].yaml')
        else:
            filenames = glob.glob(self.calibration_directory + hu.get_fleet_id()+'_omnibase_imu_calibration_data_' + '[0-9]*[0-9].yaml')
        filenames.sort()
    ...
    def qc_draw_geometries(self,geometry):
        ...
        if self.validation_run:
            ...
            vis.poll_events()
            vis.update_renderer()
            vis.capture_screen_image('omnibase_imu_calibration_calibrated_fit.png')
        else:
            ctr.rotate(0.0, 1200.0)
            #vis.run()
            vis.poll_events()
            vis.update_renderer()
            vis.capture_screen_image('omnibase_imu_calibration_uncalibrated_fit.png')
        vis.destroy_window()
    ...
    def post_fit_processing(self, final_parameters):
        ...
        if self.qc:
            ...
            if self.validation_run:
                with open('omnibase_imu_calibrated_results.yaml', 'w') as yaml_file:
                    yaml.dump(results, yaml_file, default_flow_style=False)
            else:
                with open('omnibase_imu_uncalibrated_results.yaml', 'w') as yaml_file:
                    yaml.dump(results, yaml_file, default_flow_style=False)
    ...
    try:
        ...
        if load_most_recent_opt_results or (opt_results_file_to_load is not None): 
            if load_most_recent_opt_results: 
                filenames = glob.glob(calibration_directory + hu.get_fleet_id()+'_omnibase_imu_calibration_result' + '_*[0-9].yaml')
                filenames.sort()
            ...
            filename = calibration_directory + hu.get_fleet_id()+'_omnibase_imu_calibration_result_' + time_string + '.yaml'
