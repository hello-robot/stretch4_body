...
class RobotClient(SubsystemClient):
    ...
    def __init__(self, client_id=None, ip_address=None):
        ...
        """
        SubsystemClient.__init__(
            self, name="robot", client_id=client_id, parent=None, ip_address=ip_address
        )
        ...
        for k in self.params["subsystems"]:
            ...
            if k == "end_of_arm":
                ...
                if (
                    "client_module_name" in eoa_params
                    and "client_class_name" in eoa_params
                ):
                    module_name = eoa_params["client_module_name"]
                    class_name = eoa_params["client_class_name"]

                    current_module = RobotParams.import_user_tool_module(
                        self.eoa_name, module_name, is_server=False
                    )
                else:
                    module_name = "stretch4_body.robot.robot_client"
                    class_name = (
                        self.robot_params[self.eoa_name]["py_class_name"] + "_Client"
                    )
                    ...
                    if not hasattr(current_module, class_name):
                        ...
                        # and register it into the module. EndOfArmClient is defined later in
                        # this same module, but is already fully available here at call time.
                        def dynamic_init(self_obj, parent=None):
                            EndOfArmClient.__init__(
                                self_obj, name=self.eoa_name, parent=parent
                            )

                        dynamic_class = type(
                            class_name, (EndOfArmClient,), {"__init__": dynamic_init}
                        )
                        setattr(current_module, class_name, dynamic_class)

                self.end_of_arm: EndOfArmClient = getattr(current_module, class_name)(
                    parent=self
                )
    ...
    def home(self, do_push=True, wait_on_completion=True, timeout=60, do_pull=True):
        ...
        finished, rid = self.routines.routine_robot_home(
            do_push, wait_on_completion, timeout, do_pull
        )
    ...
    def stow(self, do_push=True, wait_on_completion=True, timeout=30, do_pull=True):
        ...
        finished, rid = self.routines.routine_robot_stow(
            do_push, wait_on_completion, timeout, do_pull
        )
        if wait_on_completion:
            if not finished:
    ...
    def wait_on_motion_finish(
        self, subsystem_names, timeout=15.0, wait_on_motion_start: bool = True
    ):
        """
        ...
        def done_moving():
            for n in subsystem_names:
                if n not in self.subsystems:
                    raise ValueError(
                        f"{n} is not a subsystem in {self.subsystems.keys()}"
                    )
    ...
    def set_guarded_contact_sensitivity(self, mode_name=None):
        ...
        if mode_name not in self.get_guarded_contact_modes():
            self.logger.error(
                f"set_guarded_contact_sensitivity: Invalid mode name: {mode_name}"
            )
            return
        for s in self.subsystems:
            if (
                hasattr(self.subsystems[s], "set_guarded_contact_sensitivity")
                and s in self.params["guarded_contact"][mode_name]
            ):
                self.subsystems[s].set_guarded_contact_sensitivity(
                    self.params["guarded_contact"][mode_name][s]
                )
    ...
    def get_guarded_contact_modes(self):
        ...
        """
        return list(
            self.robot_params["robot"]["guarded_contact"].keys()
        )  # Todo: move to server RPC for this, hack for now
...
class RoutinesClient(SubsystemClient):
    ...
    def routine_robot_stow(
        self, do_push=True, wait_on_completion=True, timeout=20, do_pull=True
    ):
        """
        ...
        """
        return self.run(
            "routine_robot_stow", do_push, wait_on_completion, timeout, do_pull
        )

    def routine_robot_home(
        self, do_push=True, wait_on_completion=True, timeout=20, do_pull=True
    ):
        """
    ...
    def routine_wrist_joint_home(
        ...
        do_pull : bool, optional
            If wait_on_completion, whether to call pull_status() within this method.
        """
        rid = self._queue_command(
            subsystem="routines",
            command="routine_wrist_joint_home",
    ...
    def routine_end_of_arm_home(
        self, do_push=True, wait_on_completion=True, timeout=30, do_pull=True
    ):
        """
        ...
        """
        return self.run(
            "routine_end_of_arm_home", do_push, wait_on_completion, timeout, do_pull
        )

    def routine_lift_home(
        self, do_push=True, wait_on_completion=True, timeout=20, do_pull=True
    ):
        """
        Home the lift.
        ...
        """
        return self.run(
            "routine_lift_home", do_push, wait_on_completion, timeout, do_pull
        )

    def routine_arm_home(
        self, do_push=True, wait_on_completion=True, timeout=20, do_pull=True
    ):
        """
        Home the arm.
        ...
        """
        return self.run(
            "routine_arm_home", do_push, wait_on_completion, timeout, do_pull
        )

    def routine_blind_dock(
        self, do_push=True, wait_on_completion=True, timeout=60, do_pull=True
    ):
        """
        ...
        """
        return self.run(
            "routine_blind_dock", do_push, wait_on_completion, timeout, do_pull
        )
...
class PowerPeriphClient(SubsystemClient):
    ...
    def set_eye_animation(self, left_idx=None, right_idx=None):
        ...
        """
        self._queue_command(
            subsystem="power_periph",
            command="set_eye_animation",
            left_idx=left_idx,
            right_idx=right_idx,
        )
...
class OmniBaseClient(SubsystemClient):
    ...
    def wheel_move_to(self, wheel_name, x_rad, v_r=None, a_r=None):
        ...
        """
        self._queue_command(
            f"{wheel_name}.omnibase", "wheel_move_to", wheel_name, x_rad, v_r, a_r
        )
    ...
    def wheel_move_by(self, wheel_name, x_rad, v_r=None, a_r=None):
        ...
        """
        self._queue_command(
            f"{wheel_name}.omnibase", "wheel_move_by", wheel_name, x_rad, v_r, a_r
        )
    ...
    def get_guarded_contact_modes(self):
        ...
        """
        return list(
            self.robot_params["hello-motor-omni-0"]["guarded_contact"].keys()
        )  # Todo: move to server RPC for this, hack for now
    ...
    def is_moving(self):
        return (
            self.status["wheel_0"]["is_mg_moving"]
...
class PrismaticJointClient(SubsystemClient):
    ...
    def home(self):
        ...
        rid = self._queue_command(
            subsystem="routines", command="routine_%s_home" % self.name
        )
        self.push_command()
        finished = self._wait_on_routine(rid, timeout=20.0)
        success = finished and self.status["routines"].get(
    ...
    def move_by(
        ...
        a_m=None,
        stiffness=None,
        req_calibration=True,
        contact_sensitivity_pos=None,
        contact_sensitivity_neg=None,
    ):
        """
        ...
        if req_calibration and not self.is_homed():
            self.logger.error(
                f"Cannot send movement command. Joint {self.name} has not been homed."
            )
            return False
        self._queue_command(
            self.name,
            ...
            a_m=a_m,
            stiffness=stiffness,
            req_calibration=req_calibration,
...
class WristJointClient(SubsystemClient):
    ...
    def __init__(
        self, joint_name: str, parent: EndOfArmClient | None = None, ip_address=None
    ):
        self.joint_name = joint_name
        self.parent = parent  # keep this here for typing
        SubsystemClient.__init__(
            self, name=joint_name, parent=parent, ip_address=ip_address
        )
    ...
    def move_by(self, x_r, v_r=None, a_r=None):
        ...
        if not self.is_homed():
            self.logger.error(
                f"Cannot send movement command. Joint {self.joint_name} has not been homed."
            )
            return False
        self._queue_command(
            f"{self.joint_name}.end_of_arm", "move_by", self.joint_name, x_r, v_r, a_r
        )
        return True
    ...
    def move_to(self, x_r, v_r=None, a_r=None):
        ...
        if not self.is_homed():
            ...
            return False
        self._queue_command(
            f"{self.joint_name}.end_of_arm", "move_to", self.joint_name, x_r, v_r, a_r
        )
        return True
    ...
    def set_velocity(self, v_r, a_r=None):
        ...
        if not self.is_homed():
            ...
            return False
        self._queue_command(
            f"{self.joint_name}.end_of_arm", "set_velocity", self.joint_name, v_r, a_r
        )
        return True
    ...
    def pose(self, p, v_r=None, a_r=None):
        ...
        if not self.is_homed():
            ...
            return False
        self._queue_command(
            f"{self.joint_name}.end_of_arm", "pose", self.joint_name, p, v_r, a_r
        )
        return True
    ...
    def disable_torque(self):
        ...
        """
        self._queue_command(
            f"{self.joint_name}.end_of_arm", "disable_torque", self.joint_name
        )
    ...
    def enable_torque(self):
        ...
        """
        self._queue_command(
            f"{self.joint_name}.end_of_arm", "enable_torque", self.joint_name
        )
    ...
    def pretty_print(self):
        ...
        print(
            "Hardware Error: Input Voltage Error: ", self.status["input_voltage_error"]
        )
        print("Hardware Error: Overheating Error: ", self.status["overtemp_error"])
        print(
            "Hardware Error: Motor Encoder Error: ", self.status["motor_encoder_error"]
        )
...
class WristYawClient(WristJointClient):
    ...
    def __init__(self, parent: EndOfArmClient | None = None, ip_address=None):
        WristJointClient.__init__(
            self, joint_name="wrist_yaw", parent=parent, ip_address=ip_address
        )
...
class WristRollClient(WristJointClient):
    ...
    def __init__(self, parent: EndOfArmClient | None = None, ip_address=None):
        WristJointClient.__init__(
            self, joint_name="wrist_roll", parent=parent, ip_address=ip_address
        )
...
class WristPitchClient(WristJointClient):
    ...
    def __init__(self, parent: EndOfArmClient | None = None, ip_address=None):
        WristJointClient.__init__(
            self, joint_name="wrist_pitch", parent=parent, ip_address=ip_address
        )
...
class ToolJointClient(WristJointClient):
    ...
    def __init__(
        ...
        parent: EndOfArmClient | None = None,
        ip_address=None,
    ):
        WristJointClient.__init__(
            self, joint_name=metadata.joint_name, parent=parent, ip_address=ip_address
        )
...
class EndOfArmClient(SubsystemClient):
    ...
    def __init__(self, name="end_of_arm", parent=None):
        ...
        for joint in self.joints:
            ...
            device_params = (
                self.robot_params[self.name].get("devices", {}).get(joint, {})
            )
    ...
    def move_by(self, joint, x_r, v_r=None, a_r=None):
        ...
        if not self.is_homed(joint):
            self.logger.error(
                f"Cannot send movement command. Joint {joint} has not been homed."
            )
            return False
    ...
    def home(self, wait_on_completion=True, timeout=45):
        ...
        """
        rid = self._queue_command(
            subsystem="routines", command="routine_end_of_arm_home"
        )
        self.logger.info("Homing %s ..." % self.name)
        self.push_command()
        if wait_on_completion:
    ...
    def stow(self, wait_on_completion=True, timeout=20):
        ...
        """
        rid = self._queue_command(
            subsystem="routines", command="routine_end_of_arm_stow"
        )
    ...
    def is_homed(self, joint=None):
        if joint is None:
            ...
            for j in self.joints:
                req_cal = (
                    self.robot_params[self.name]
                    .get("devices", {})
                    .get(j, {})
                    .get("req_calibration", True)
                )
                req = req and (
                    not req_cal or self.status.get(j, {}).get("pos_calibrated", False)
                )
            return req
        else:
            req_cal = (
            ...
            return not req_cal or self.status.get(joint, {}).get(
...
class EOA_Wrist_DW4_Tool_Calibration_Client(EndOfArmClient):
    ...
    def __init__(self, parent=None):
        EndOfArmClient.__init__(
            self, name="eoa_wrist_dw4_tool_calibration", parent=parent
        )
        ...
        if r.startup():
            ...
            try:
                while True:  # time.time()-ts<3.0):
                    ...
                    s = r.status
                    print(
                        "RobotServer : Runtime %.8f (s) | Rate %.2f (Hz): "
                        % (
                            s["control_loop"]["execution_time_s"],
                            s["control_loop"]["curr_rate_hz"],
                        )
                    )
        ...
        if r.startup():
            ...
            try:
                while True:  # time.time()-ts<3.0):
                    ...
                    if s is not None:
                        print(
                            "RobotServer : Runtime %.2f (s) | Rate %.2f (Hz): "
                            % (
                                s["control_loop"]["loop_clock"],
                                s["control_loop"]["loop_rate_actual"],
                            )
                        )
                    else:
                        time.sleep(0.1)
